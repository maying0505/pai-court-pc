...
import { Input, Modal, Form, Button, message, Spin, Divider, Table } from 'antd';
...
class HousePropertyForm extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            standardDetail_others: cloneDeep(showJson.standardDetail_others_house),
    ...
    componentDidMount() { //预加载数据
        ...
        if (sessionStorage.getItem('pageName') === 'court_query_detail') {//是否为标的查询
            this.setState({
                isQuery: true
            })
    ...
    jsonChoose = (path) => {
        ...
        if (path.indexOf('house_property') !== -1) {
            ...
            if (sessionStorage.getItem('status') === '4') {
                let standardDetail_othersBefore = cloneDeep(showJson.standardDetail_others_house);
                standardDetail_othersBefore[0] = {
                    label: '户型',
                    child: [
                        {
                            fieldName: 'room',
                ...
                this.setState({
                    standardDetail_others: standardDetail_othersBefore
                },function(){
                    console.log('standardDetail_others',this.state.standardDetail_others)
                })
            } else {
                this.setState({
                    standardDetail_others: cloneDeep(showJson.standardDetail_others_house)
    ...
    _releaseGetData = async () => { 
        ...
        try {
            ...
            if (result.success) {
                ...
                if (this.state.thePath.indexOf('house_property') !== -1) {
    ...
    _getKeyListData = async (id) => { 
        try {
            const result = await HTTP.courtHistoryList({
                assetId: id,
            });
            this.setState({
                tableLoading: false,
    ...
    render() {
        const { tableListDataSource, keyColumns, tableLoading, thePath, assetAddReleaseVisible, curTab, HeaderTabs, userId, auctionLink, operationProcessId, isQuery, standardDetailPeripheralEnvironment_1, standardDetailPeripheralEnvironment_2, standardDetailDistrictMatching_2, standardDetailDistrictMatching_1, isAudit, isRelease, isShow, longitude, latitude, address, assetBaseId, imgDefaultData, detailData, imgUploadArray, othersDetail, standardDetailOtherFee, standardDetailOtherFee_dataSources, auctionDetail, standardDetail, standardDetailTaxFee_buy, standardDetailTaxFee_sell, standardDetailTaxFee_common, standardDetail_others } = this.state;
        const { form } = this.props;
        const { getFieldDecorator } = form;
        return (
            <Spin size="large" spinning={this.state.loading} style={{minHeight: '200px'}}>
                {isRelease && <div className="margin_t_20">
                    <span>拍卖链接：</span>
                    <span style={{width: '200px',display: 'inline-block',marginRight: '20px'}}><Input onChange={this.getLink} value={auctionLink}/></span>
                ...
                {!isRelease &&
                    <Form layout="inline" id="wy-scroll-layout" className={this.state.isShow ? 'detail_box' : ''}> 
                        ...
                        <DetailModuleBox title="钥匙记录" id="key-record" className={isShow ? 'show_process border_b border_r': ''}>
                            <Table bordered rowKey="id" columns={keyColumns} 
