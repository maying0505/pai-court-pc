...
import { Cascader, Icon, Row, Col, Form, Input, Select, DatePicker, message } from 'antd'
...
class FormSet extends React.Component {
    ...
    propsGet = (props) => {
        if (props.defaultValues !== this.state.defaultValues){
            console.log('warrants:',props.defaultValues,this.state.defaultValues)
    ...
    _outsideOperatorListFetch = async (params) => {
        ...
        try {
            ...
            console.log('outsideOperatorList', result);
            if (result.success) {
                this.setState({
    ...
    inputBoxShow = (key,style,value,length,placeholder,width,mode,format,disabled,disabledText) =>{
        console.log('disabled:',disabled,style)
        let text = '';
        if (style === 'input') {
            ...
            text =<Select disabled={disabled ? disabled : false} allowClear={true} mode={mode ? "multiple" : ''} key={key} placeholder={placeholder ? placeholder :"请选择"} style={{width: `${width ? width :90/length}%`}}>
                {
            ...
            text = style === 'court' ? <Cascader disabled={disabled ? disabled : false} style={{width: `${width ? width :90/length}%`}} placeholder={placeholder ? placeholder :"请选择"} options={this.state.courtList}/> :
            <Cascader disabled={disabled ? disabled : false} style={{width: `${width ? width :90/length}%`}} placeholder={placeholder ? placeholder :"请选择"} options={this.state.areaList}/>
    ...
    _addHtmlText = (child,isRequired) => {
        ...
        const initialValueDo = (index,item) => {
            ...
            if (warrantSituation[index] && warrantSituation[index][item.fieldName]) {
                console.log('text',item.fieldName)
        ...
        return (
            <span key={index} style={{position: 'relative'}}>
                {child.map((item,key)=>
                    <span key={key}>
                        ...
                        {getFieldDecorator(`${item.fieldName}[${k}]`,{
                            ...
                            this.inputBoxShow(key,item.style,item.value,child.length,item.placeholder,item.width,item.mode,item.format,item.disabled && item.disabledText && item.disabledText[index]? true : false, item.disabledText && item.disabledText[index]? item.disabledText[index] : false)
                        )}
                ...
                {index > 1 && <Icon type="close" className="add_icon" onClick={() => this._remove(k)}/>}
            </span>
            );
        })
        return formItems;
                                ...
                                <span key={key}>
                                   ...
                                   {getFieldDecorator(item.fieldName,{
                                       ...
                                       rules: [{ required: isRequired, message: '必填项!' }]
                                        ...
                                        this.inputBoxShow(key,item.style,item.value,child.length,item.placeholder,item.width,item.mode,item.format,!item.disabled? false : true)
                                    )}
                                    {item.unit && <span className="number-bettw">
                                        {item.unit}
                                    </span>}
    ...
    _addShowHtmlText = (child) => {
        ...
        console.log('_addShowHtmlText',text)
        return text
    }
